	...
	public GameWorld() {
		...
		for (int row=0; row<GRID_HEIGHT; row++){
			for (int col=0; col<GRID_WIDTH; col++){
				if (row==4||row==5||row==8||row==9||row==12||row==13){buildable[row][col] = false;} //horizontal paths 
				else if ((col==0||col==1) && (row<6 || (row>=8&&row<14))){buildable[row][col] = false;} //vertical left side
				else if ((col==18||col==19) && (row >=4 && row<10)){buildable[row][col] = false;} //vertical right side
				else if (row>=12&&col>=14){buildable[row][col]=false;} // tower
				else {buildable[row][col]=true;}
			}
		}
